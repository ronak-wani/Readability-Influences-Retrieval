...
class RAG:
    ...
    def docs_to_indices(self, docs):
        indices = []
        for d in docs[:TOP_K]:
            for i, doc in enumerate(self.docs):
                if (
                    doc.page_content == d.page_content
                    and doc.metadata.get("title") == d.metadata.get("title")
                    and doc.metadata.get("level") == d.metadata.get("level")
                ):
                    indices.append(i)
                    break
        return indices
